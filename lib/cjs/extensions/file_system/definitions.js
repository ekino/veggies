...
const install = () => {
  (0, import_cucumber.Given)(/^(?:I )?create directory (.+)$/, function(directory) {
    return this.fileSystem.createDirectory(this.cli.getCwd(), directory);
  });
  (0, import_cucumber.Given)(/^(?:I )?remove (?:file|directory) (.+)$/, function(fileOrDirectory) {
    return this.fileSystem.remove(this.cli.getCwd(), fileOrDirectory);
  });
  (0, import_cucumber.Then)(/^(file|directory) (.+) should (not )?exist$/, function(type, file, flag) {
